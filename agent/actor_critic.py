...
class Critic(nn.Module):
    # Triplet Deep Deterministic architecture
    def __init__(self, state_dim, action_dim):
        ...
        # Q3 Architecture (Additional for TDD-ND overestimation bias correction)
...
class TDD_ND_Agent:
    ...
    def train(self, replay_buffer):
        ...
        with torch.no_grad():
            ...
            next_action = (self.actor_target(next_state) + noise).clamp(-self.config.max_action, self.config.max_action)

            # Triplet Target Q-Values: y = r + gamma * [beta * min(Q1, Q2) + (1-beta) * Q3]
            target_Q1, target_Q2, target_Q3 = self.critic_target(next_state, next_action)
            
            min_Q1_Q2 = torch.min(target_Q1, target_Q2)
            target_Q = self.config.beta * min_Q1_Q2 + (1.0 - self.config.beta) * target_Q3
